...
async function initConfigFunctions(
    ...
    const { height: defaultBottomHeight, depth: defaultbottomFullDepth } = bottomDimensions;
    const { height: defaultTopHeight, depth: defaultTopDepth } = topDimensions;
    const { height: defaultFullHeight } = fullDimensions;
    ...
    let currentBasePrice = currentBaseData.price;
    let currentFramePrice = currentFrameData.price;
    ...
    function openLoginModal() {
        ...
        toggleLoginRegisterModalContent(loginFormHtml, registerFormHtml);
    ...
    function toggleLoginRegisterModalContent(loginFormHtml, registerFormHtml) {
    ...
    function changeTotals() {
        let newTotal = 0;
        console.log(modelChildrenList)
        for(let i = 0; i < modelChildrenList.length; i++) {
            const item = modelChildrenList[i];
            console.log(item)
            const itemPrice = parseFloat(item.display_price);
            const totalItemPrice = itemPrice + (currentBasePrice > 0 ? currentBasePrice : 1) + (currentFramePrice > 0 ? currentFramePrice : 1);
        ...
        function typesEditInit() {
            ...
            furnitureItems.forEach(furnitureItem => {
                ...
                editBtn.addEventListener('click', function(e) {
                    ...
                    createEditModal(furnitureItem, itemData, furnitureId, type, modelFileSrc, itemWidth, displayPrice, regularPrice, discountPrice);
    ...
    function initAddFurnitureMethod() {
        ...
        addFurnitureButtons.forEach(furnitureButton => {
            ...
            furnitureButton.addEventListener('click', async function() {
                ...
                const { itemHeight, itemDepth } = getItemDimensions(type);

                await addGLBModel(modelFileSrc, type, furnitureId, customId, displayPrice, regularPrice, discountPrice, itemHeight, itemDepth, itemWidth, defaultBottomReferenceY);
            
                const [itemHtml] = await addFurnitureItem(
                    customId, 
                    furnitureId, 
                    itemWidth,
                    itemHeight,
                ...
                if(itemHtml) {
                    myItemsList.insertAdjacentHTML('beforeend', itemHtml);
    ...
    function editItemButtonTrigger(furnitureItem) {
        ...
        editButton.addEventListener('click', function(e) {
            ...
            const itemWidth = furnitureItem.getAttribute('data-item_width');
            createEditModal(furnitureItem, itemData, furnitureId, type, modelFileSrc, itemWidth, displayPrice, regularPrice, discountPrice, false);
        });

    }

    function removeItemButtonTrigger(furnitureItem) {
    ...
    function createEditModal(item, data, furnitureId, type, modelFileSrc, width, displayPrice, regularPrice, discountPrice, actionTypeAdd = true) {
        ...
        if(addBtn) {
            addBtn.addEventListener('click', async function() {
                ...
                const customId = Date.now();
                const { itemHeight, itemDepth } = getItemDimensions(type);
                addGLBModel(modelFileSrc, type, furnitureId, customId, displayPrice, regularPrice, discountPrice, itemHeight, itemDepth, itemWidth, defaultBottomReferenceY);
    ...
    function init3dModel(model3dContainer) {
        ...
        const frustumHeight = modelRoomHeight * 1.2; // add 20% margin
        const frustumWidth = frustumHeight * aspect;
        threeJSCamera = new THREE.OrthographicCamera(
    ...
    function init3dModel2(model3dContainer) {
        model3dContainer.innerHTML = '';
    ...
    function addGLBModel(urlSrc, furnitureType, furnitureId, customId, displayPrice, regularPrice, discountPrice, itemHeight, itemDepth, itemWidth) {
        ...
        loader.load(
            ...
            function (childGltf) {
                ...
                const yPosition = furnitureType === DIMENSION_TYPE_TOP
                    ? calculateTopSpaceIn3dModel(finalParentBox, objHeight)
                    : finalParentBox.min.y;
                ...
                const isFitting = checkIfAbleToDragChildToPosition(childMeshGroup, scaledChildBox);
                childMeshGroup.userData.isFitting = isFitting;
             console.log("isFitting isFitting isFitting: ", isFitting)
                if (!isFitting) {
                ...
                console.log(`✅ Loaded model: ${urlSrc}`);
            },
            (xhr) => {
    ...
    function addExistingGLBModel(id, urlSrc, furnitureType, furnitureId, customId, itemWidth, originalSize, scaledSize, savedPosition, isFitting) {
        ...
        loader.load(
            ...
            function (childGltf) {
                ...
                dragControls.addEventListener('drag', event => {
                    const obj = event.object;
                    let unavailablePosiotion = null;
                    if(roomType == ROOM_TYPE_WITH_CORNER) {
                        unavailablePosiotion = setCornerChildenDragging(obj);
                    } else {
                        unavailablePosiotion = setSingleWallChilderDragging(obj);
                    }

                    obj.userData.isFitting = !unavailablePosiotion;
                    if (unavailablePosiotion) {
                        obj.traverse(child => {
                            if (child.isMesh) {
                                child.material.color.set(0xff0000);
                                child.material.transparent = true;
    ...
    function initModelClickEvent(threeJSRendered) {
        const raycaster = new THREE.Raycaster();
        const mouse = new THREE.Vector2();
        threeJSRendered.domElement.addEventListener('click', onClick, false);
        ...
        function onClick(event) {
            ...
            if (intersects.length > 0) {
                ...
                if (obj.userData && obj.userData.customId) {
                    console.log("Clicked customId:", obj.userData.customId);
        ...
        function onChildClick(childMeshGroup) {
            ...
            openEditModal(userData, customId);
            ...
            console.log("Selected object:", childMeshGroup.userData);
        }

        function openEditModal(userData, customId) {
            ...
            editContainerRemove();
            createEditModal(furnitureItem, itemData, furnitureId, type, modelFileSrc, width, displayPrice, regularPrice, discountPrice);
        }
        ...
        function createFurnitureItemControls(childMeshGroup, userData, customId) {
            ...
            // attach events
            model3dContainer.addEventListener("click", e => {
                const action = e.target.dataset.action_type;
                console.log(action)
                if (!action) return;
                console.log(`Action: ${action} on`, childMeshGroup.userData.customId);
...
console.log("TEEEEEEEEEEEST")
    ...
    function changeModelChildrenDimensions() {
        ...
        for(let i = 0; i < children.length; i++) {
            ...
            const yPosition = childFurnitureType === DIMENSION_TYPE_TOP
                ...
                : parentBox.min.y;
            const zPosition = parentBox.min.z + (scaledChildSize.z / 2) + 1; 
            ...
            if(roomType === ROOM_TYPE_WITH_CORNER) {
                ...
                const scaledDepth = childObj.userData.originalSize.z * childObj.scale.z;
                // const childObjBox = new THREE.Box3().setFromObject(childObj);
                ...
                if (Math.abs(childObj.rotation.y) > 0.0001) {
                    ...
                    objWorldPos.x = parentBoxWorld.min.x + scaledDepth / 2 + 1;
    ...
    function calculateTopSpaceIn3dModel(parentBoxWorld) {
        const bottom = (parentBoxWorld.min.y) + defaultBottomReferenceY;
    ...
    function getDefaultBottomHeightWithSpace() {
        ...
        return modelRoomHeight * bottomPercent;  // this is already in scene units
    }
    ...
    function setSingleWallChilderDragging(obj) {
        ...
        objWorldPos.z = parentBoxWorld.min.z + (objDepth / 2) + 1; 

        objWorldPos.x = Math.max(
            parentBoxWorld.min.x + objWidth / 2  + 1,
    ...
    function setCornerChildenDragging(obj) {
        ...
        if (obj.position.x < forwardThreshold - 0.01) {
            ...
            objWorldPos.x = parentBoxWorld.min.x - objDepth / 2 + 1;
        } else {
            obj.rotation.y = 0;
            objWorldPos.z = parentBoxWorld.min.z + objDepth / 2 + 1;
        }
        ...
        objWorldPos.z = Math.max(
            parentBoxWorld.min.z + objDepth / 2  + 1,
    ...
    function checkIfAbleToDragChildToPosition(currentObj, currentBox) {
        ...
        if(roomType === ROOM_TYPE_SINGLE_WALL) {
            ...
            while(i < childrenCount) {
                ...
                if(!isRotated) {
                    ...
                    const childZMin = childBoxMin.z;
                    const childZMax = childBoxMax.z;
...
async function addFurnitureItem(
    ...
    try {
        ...
        if(!jsonData.success || !jsonData.data?.item_html) return '';
        const data = jsonData.data;
