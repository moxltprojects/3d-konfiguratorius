...
//         });
    ...
    function addGLBModel(urlSrc, furnitureType, furnitureId, customId, displayPrice, regularPrice, discountPrice, displayPriceCm3, itemHeight, itemDepth, itemWidth, itemHeightMin, itemDepthMin, itemWidthMin, triggerDupItem = false) {
        return new Promise((resolve, reject) => {
            ...
            loader.load(
                urlSrc,
                function (childGltf) {
                    let meshGroup = childGltf.scene || childGltf.scenes[0];
                    const childMeshList = [];
                    ...
                    let childMeshGroup = new THREE.Group();
                        // modelScene.add(childMeshGroup);
                    childMeshList.forEach(mesh => {
                        childMeshGroup.attach(mesh);
                    });
                    childMeshGroup.userData.furnitureId = furnitureId;
                    childMeshGroup.userData.customId = customId;
                    childMeshGroup.userData.furnitureType = furnitureType;
                    ...
                    const childBox = new THREE.Box3().setFromObject(childMeshGroup);
                    const { scaledChildBox, scaledChildSize } = normalizeAndPlaceMeshGroup(childMeshGroup, childBox, furnitureType, dims);

                    /******** 2.save position and size********/
                    const placedWorldPos = new THREE.Vector3();
                    childMeshGroup.getWorldPosition(placedWorldPos);
                    childMeshGroup.userData.savedPosition = placedWorldPos.clone();
                    childMeshGroup.userData.scaledSize = scaledChildSize;
                    /******** end save position ********/

                    /******* enable dragging *********/

                    const isFittingItem = initModelDragging(childMeshGroup, childBox, scaledChildBox, scaledChildSize, customId, furnitureType);
    ...
    return new Promise((resolve, reject) => {
        ...
        loader.load(
            urlSrc,
            function (childGltf) {
                const meshGroup = childGltf.scene || childGltf.scenes[0];
                const childMeshList = [];

                // Traverse all meshes and setup geometry/material
                meshGroup.traverse(node => {
                    if (node.isMesh) {
                        const geo = node.geometry;
                        if (geo && geo.attributes.position) {
                ...
                // ------------------------
                // Logical group for metadata (not added to scene)
                // ------------------------
                const childMeshGroupData = {
                    meshes: childMeshList,
                    userData: {
                        furnitureId,
                        customId,
                        furnitureType,
                ...
                childMeshList.forEach(mesh => {
                    ...
                    mesh.applyMatrix4(mesh.matrixWorld); // bake world transform into local
                    modelScene.add(mesh);
                ...
                childMeshGroupData.userData.originalBox = childBox.clone();

                // ------------------------
                // Normalize, scale, and place
                // ------------------------
                const { scaledChildBox, scaledChildSize } = normalizeAndPlaceMeshGroupForMeshes(childMeshList, childBox, furnitureType, dims);

                // Save positions in metadata
                const placedWorldPos = new THREE.Vector3();
                childBox.getCenter(placedWorldPos);
                childMeshGroupData.userData.savedPosition = placedWorldPos.clone();
                childMeshGroupData.userData.scaledSize = scaledChildSize;

                // ------------------------
                // Enable dragging for all meshes
                // ------------------------
                const isFittingItem = initModelDraggingForMeshes(childMeshList, childBox, scaledChildBox, scaledChildSize, customId, furnitureType);
                const itemPositionMm = getItemPosition(placedWorldPos.clone(), scaledChildSize);
                childMeshGroupData.userData.positionMm = itemPositionMm;
                ...
                const dbData = {
                    ...
                    width: itemWidth,
                    furniture_position_mm: itemPositionMm,
                    model_original_size: JSON.stringify(childMeshGroupData.userData.originalSize || {}),
                    model_scaled_size: JSON.stringify(childMeshGroupData.userData.scaledSize || {}),
                    model_position: JSON.stringify(childMeshGroupData.userData.savedPosition),
                    is_fitting: isFittingItem
                };
                ...
                if (visibleModelDimensionArrows) {
                    createFurnitureDimensionArrows(childMeshList, room3DGroup);
                }
...
// ------------------------
// Normalize, scale, and place for multiple meshes
// ------------------------
function normalizeAndPlaceMeshGroupForMeshes(meshList, targetBox, furnitureType, dims) {
    const [heightPx, depthPx, widthPx] = dims;

    // Compute combined bounding box
    const combinedBox = targetBox.clone();
    const childSize = new THREE.Vector3();
    combinedBox.getSize(childSize);

    // Save original size per mesh (optional)
    meshList.forEach(mesh => {
        mesh.userData.originalSize = childSize.clone();
    });

    // Center and scale each mesh
    meshList.forEach(mesh => {
        const bbox = new THREE.Box3().setFromObject(mesh);
        const size = new THREE.Vector3();
        bbox.getSize(size);
        ...
        mesh.position.sub(center); // center
        mesh.scale.set(widthPx / size.x, heightPx / size.y, depthPx / size.z);

        mesh.updateMatrix();
        mesh.updateMatrixWorld(true);
    });

    // Compute new scaled combined box
    const scaledChildBox = new THREE.Box3();
    meshList.forEach(mesh => scaledChildBox.expandByObject(mesh));
    const scaledChildSize = new THREE.Vector3();
    scaledChildBox.getSize(scaledChildSize);
    ...
    const yPosition = (furnitureType.includes(DIMENSION_TYPE_TOP)
        ? calculateTopSpaceIn3dModel(finalParentBox)
        : finalParentBox.min.y + floorThickness) + scaledChildSize.y / 2;
    const backZ = finalParentBox.min.z + scaledChildSize.z / 2 + wallThickness;

    // Apply translation to each mesh
    meshList.forEach(mesh => {
        const meshBox = new THREE.Box3().setFromObject(mesh);
        const meshCenter = meshBox.getCenter(new THREE.Vector3());
        mesh.position.add(new THREE.Vector3(
            leftX - meshCenter.x,
            yPosition - meshCenter.y,
            backZ - meshCenter.z
        ));
    });

    return { scaledChildBox, scaledChildSize };
}

// ------------------------
// Init dragging for multiple meshes
// ------------------------
function initModelDraggingForMeshes(meshList, childBox, scaledChildBox, scaledChildSize, customId, furnitureType) {
    let isFittingItem = true;

    meshList.forEach(mesh => {
        if (!furnitureType.includes('corner')) {
            isFittingItem = checkIfAbleToDragChildToPosition(mesh, scaledChildBox);
            mesh.userData.isFitting = isFittingItem;
    ...
    const dragControls = new DragControls(meshList, threeJSCamera, threeJSRendered.domElement);
    dragControls.transformGroup = true;

    dragControls.addEventListener('dragstart', () => { threeJSControls.enabled = false; });
    dragControls.addEventListener('dragend', event => {
        ...
        // Restore material if fitting
        if (obj.userData.isFitting && obj.userData.originalMaterial) {
            obj.material.color.copy(obj.userData.originalMaterial.color);
            obj.material.opacity = obj.userData.originalMaterial.opacity;
            obj.material.transparent = obj.userData.originalMaterial.transparent;
        }
    });
    ...
    dragControls.addEventListener('drag', event => {
        ...
        if (roomType === ROOM_TYPE_WITH_CORNER) isFitting = setCornerChildenDragging(obj);
        else isFitting = setSingleWallChilderDragging(obj);

        obj.userData.isFitting = isFitting;

        obj.material.color.set(isFitting ? 0x00ff00 : 0xff0000);
        obj.material.transparent = true;
        obj.material.opacity = 0.5;

        if (visibleModelDimensionArrows) {
            createFurnitureDimensionArrows(meshList, room3DGroup);
        }
    });
