...
async function initConfigFunctions(
    ...
    let fullFurnitureHeight = 0;
    let bottomFurnitureHeight = 0;
    let topFurnitureHeight = 0;
    let fullFurnitureDepth = 0;
    let bottomFurnitureDepth = 0;
    let topFurnitureDepth = 0;
...
//     }
    ...
    function initAddFurnitureMethod() {
        ...
        addFurnitureButtons.forEach(furnitureButton => {
            ...
            furnitureButton.addEventListener('click', async function() {
                ...
                const { itemHeight, modelHeight, itemMinHeight, itemDepth, modelDepth, itemMinDepth } = getItemDimensions(type);
                ...
                const {itemPositionMm} = await addGLBModel(modelFileSrc, type, furnitureId, customId, displayPrice, regularPrice, discountPrice, displayPriceCm3, itemHeight, modelHeight, itemDepth, modelDepth, itemWidth, itemMinHeight, itemMinDepth, itemMinWidth);
    ...
    async function duplicateFurniture(currentCustomId, triggerDupItem = false) {
        ...
        const {itemPositionMm} = await addGLBModel(modelFileSrc, type, furnitureId, customId, displayPrice, regularPrice, discountPrice, displayPriceCm3, itemHeight, itemModelHeight, itemDepth, itemModelDepth, itemWidth, itemMinHeight, itemMinDepth, itemMinWidth, triggerDupItem);
    ...
    function getItemDimensions(type) {
        ...
        switch(type) {
            case DIMENSION_TYPE_BOTTOM: {
                ...
                bottomFurnitureDepth = get3DItemHeight(itemDepth, modelRoomDepth);
                bottomFurnitureHeight = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = bottomFurnitureHeight;
                modelDepth = bottomFurnitureDepth;
            ...
            case DIMENSION_TYPE_TOP: {
                ...
                topFurnitureDepth = get3DItemHeight(itemDepth, modelRoomDepth);
                topFurnitureHeight = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = topFurnitureHeight;
                modelDepth = topFurnitureDepth;
                break;
            }
            case DIMENSION_TYPE_TOP_CORNER: {
                itemHeight = dimensions.topHeight;
            ...
            case DIMENSION_TYPE_FULL: {
                ...
                fullFurnitureDepth = get3DItemHeight(itemDepth, modelRoomDepth);
                fullFurnitureHeight = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = fullFurnitureHeight;
                modelDepth = fullFurnitureDepth;
    ...
    //     const near = 0.1;
    //     const far = Math.max(modelRoomWidth, modelRoomDepth, modelRoomHeight) * 10;
    ...
    //         side: THREE.DoubleSide
    //     }); 
    //     const wallHeightWithFloor = modelRoomHeight + floorThickness;

    //     if(roomType === ROOM_TYPE_WITH_CORNER) {
    //         const leftWallGeometry = new THREE.BoxGeometry(wallThickness, wallHeightWithFloor, modelRoomDepth + floorThickness);
    //         const leftWall = new THREE.Mesh(leftWallGeometry, wallMaterial);
    //         leftWall.position.set(-modelRoomWidth / 2 - wallThickness / 2, wallHeightWithFloor / 2 - floorThickness, -floorThickness / 2);
    //         leftWall.receiveShadow = true;
    //         leftWall.castShadow  = true;
    //         room3DGroup.add(leftWall);
    //     }

    //     const rightWallGeometry = new THREE.BoxGeometry(modelRoomWidth, wallHeightWithFloor, wallThickness);
    //     const rightWall = new THREE.Mesh(rightWallGeometry, wallMaterial);
    //     rightWall.position.set(0, wallHeightWithFloor / 2 - floorThickness, -modelRoomDepth / 2 - wallThickness / 2);
    ...
    //     /**** RANDOM SUN SPOTLIGHT ****/
    //     function addRandomSunlight() {
    //         // Random position above ceiling
    //         const sunX = (Math.random() - 0.5) * modelRoomWidth;
    //         const sunZ = (Math.random() - 0.5) * modelRoomDepth;
    //         const sunY = modelRoomHeight + 5; // slightly above ceiling

    //         // Spotlight parameters
    //         const sunColor = 0xffffff;   // white sunlight
    //         const sunIntensity = 5.0;    // adjust brightness
    //         const sunDistance = Math.max(modelRoomWidth, modelRoomDepth) * 2;
    //         const sunAngle = Math.PI / 6; // ~30° cone
    //         const sunPenumbra = 0.3;

    //         const sunLight = new THREE.SpotLight(sunColor, sunIntensity, sunDistance, sunAngle, sunPenumbra);
    //         sunLight.position.set(sunX, sunY, sunZ);

    //         // Target random point on the floor
    //         const targetX = (Math.random() - 0.5) * modelRoomWidth;
    //         const targetZ = (Math.random() - 0.5) * modelRoomDepth;
    ...
    //         sunLight.shadow.camera.far = modelRoomHeight * 3;

    //         // Add spotlight and target to scene
    //         modelScene.add(sunLight);
    //         modelScene.add(sunLight.target);

    //         // Optional: add small visible bulb for effect
    //         const bulbGeom = new THREE.SphereGeometry(0.1, 16, 8);
    ...
    //         bulbMesh.position.copy(sunLight.position);
    //         modelScene.add(bulbMesh);

    //         // Ensure floor receives shadows
    //         floor.receiveShadow = true;
    //     }
    //     addRandomSunlight();
    // //     const sunColor = 0xffffff;
    // // const sunIntensity = 5;         // realistic for rendering
    // // const sunDistance = modelRoomWidth * 2; // how far the light reaches
    // // const sunAngle = Math.PI / 6;   // narrow cone
    // // const sunPenumbra = 0.3;
    ...
    // // // **Correct positioning above room**
    // // sunLight.position.set(
    // //     (Math.random() - 0.5) * modelRoomWidth, // random X above floor
    // //     modelRoomHeight + 10,                     // above ceiling
    // //     (Math.random() - 0.5) * modelRoomDepth  // random Z above floor
    // // );

    // // // Target somewhere on the floor
    // // sunLight.target.position.set(
    // //     (Math.random() - 0.5) * modelRoomWidth,
    // //     0, // floor
    // //     (Math.random() - 0.5) * modelRoomDepth
    // // );

    // // modelScene.add(sunLight);
    // // modelScene.add(sunLight.target);

    //     // // Optional bulb mesh to visualize light source
    //     // const bulbGeom = new THREE.SphereGeometry(0.1, 16, 8);
    //     // const bulbMat = new THREE.MeshStandardMaterial({ emissive: 0xffffff, emissiveIntensity: 5, color: 0x000000 });
    //     // const bulbMesh = new THREE.Mesh(bulbGeom, bulbMat);
    //     // bulbMesh.position.copy(sunLight.position);
    //     // modelScene.add(bulbMesh);
    ...
    //     threeJSControls.target.set(0, roomHeight / 2, 0);
        ...
        function renderLights() {
            ...
            for (let i = 0; i <= maxNum; i += 0.1) {
                ...
                spot.position.set(
                    ...
                    modelRoomHeight - wallThickness * 2,
                    modelRoomDepth / 4 * -1
                );
            ...
            for (let i = 0; i <= maxNum; i += 0.1) {
                ...
                spot.position.set(
                    (modelRoomWidth + modelRoomWidth * i),
                    modelRoomHeight - wallThickness * 2,
                    (modelRoomDepth / 2 * -1) + fullFurnitureDepth
                )
    ...
    function addGLBModel(urlSrc, furnitureType, furnitureId, customId, displayPrice, regularPrice, discountPrice, displayPriceCm3, itemHeight, itemModelHeight, itemDepth, itemModelDepth, itemWidth, itemHeightMin, itemDepthMin, itemWidthMin, triggerDupItem = false) {
        return new Promise((resolve, reject) => {
            const itemModelWidth = get3DItemWidth(itemWidth, modelRoomWidth);
            ...
            loader.load(
                ...
                function (childGltf) {
                    ...
                    wrapper.userData.furnitureId = furnitureId;
                    ...
                    const { scaledChildBox, scaledChildSize } = normalizeAndPlaceMeshGroup(wrapper, childBox, furnitureType, itemModelWidth, itemModelHeight, itemModelDepth);
                ...
                //    addFrontTopShade(wrapper, scaledChildSize);
                    ...
                    const isFittingItem = initModelDragging(wrapper, childBox, scaledChildBox, scaledChildSize, customId, furnitureType);
    ...
    function addFrontTopShade(wrapper, scaledChildSize) {
         // Create a thin box to act as the shade
        const shadeThickness = scaledChildSize.y * 0.05; // thin height
        const shadeDepth = scaledChildSize.z * 0.1;      // small front overhang
        const shadeWidth = scaledChildSize.x * 0.8;      // slightly smaller than furniture width

        const shadeGeometry = new THREE.BoxGeometry(shadeWidth, shadeThickness, shadeDepth);
        ...
        // Position the shade at the very top-front edge
        shadeMesh.position.set(
            0, // centered left-right
            scaledChildSize.y + shadeThickness / 2, // just above top surface
            scaledChildSize.z / 2 - shadeDepth / 2 // front edge
        );

        wrapper.add(shadeMesh);

        // Add a small soft white light (not a beam)
        const light = new THREE.PointLight(0xffffff, 2, scaledChildSize.z * 2);
        light.position.set(
            0,
            scaledChildSize.y + shadeThickness / 2,
            scaledChildSize.z / 2 - shadeDepth
        );
        wrapper.add(light);
    }
    ...
    function normalizeAndPlaceMeshGroup(childMeshGroup, targetBox, furnitureType, widthPx, heightPx, depthPx){
        ...
        //***Move to origin
        // targetBox.setFromObject(childMeshGroup);
        const center = new THREE.Vector3();
        targetBox.getCenter(center);
        childMeshGroup.position.sub(center);
        // childMeshGroup.traverse(node => {
        //     if (node.isMesh) {
        //         node.position.sub(center);  // shift geometry into center
        //     }
        // });
        ...
        // // console.log("yPosition: ", yPosition)
        // // console.log("backZ: ", backZ)
        // // // childMeshGroup.position.add(new THREE.Vector3(0, 0, 0));
    ...
    function initModelDragging(childMeshGroup, childBox, scaledChildBox, scaledChildSize, customId, furnitureType) {
        let isFittingItem = true;
...
function createBasicDisplayDimensionArrowsGood() {
    ...
    async function createBasicDisplayDimensionArrows() {
        basicDisplayScene.children.forEach(child => {
            if (child.userData.dimensions) {
                child.userData.dimensions.forEach(dim => rootGroup.remove(dim)); // remove old
            }
        });
        ...
        async function setBoxBottom() {
            if (basicDisplayBottomObj.userData.dimensions) {
                basicDisplayBottomObj.userData.dimensions.forEach(dim => rootGroup.remove(dim)); // remove old
            }
        ...
        async function setBoxTop() {
            if (basicDisplayTopObj.userData.dimensions) {
                basicDisplayTopObj.userData.dimensions.forEach(dim => rootGroup.remove(dim)); // remove old
            }
        ...
        async function setBoxFull(boxB) {
            if (basicDisplayFullObj.userData.dimensions) {
                basicDisplayFullObj.userData.dimensions.forEach(dim => rootGroup.remove(dim)); // remove old
            }
   ...
   // // Helper: simple text sprite
    ...
    function changeModelChildrenDimensions() {
        ...
        for(let i = 0; i < children.length; i++) {
            ...
            const { itemHeight, modelHeight, itemDepth, modelDepth } = getItemDimensions(childFurnitureType);

            let widthPx = get3DItemWidth(itemWidth, modelRoomWidth)
    ...
    function get3DItemDimensions(itemHeight, itemDepth, itemWidth, parentHeight, parentDepth, parentWidth) {
        const heightWorld = get3DItemHeight(itemHeight, parentHeight);
        const depthWorld = get3DIteDepth(itemDepth, parentDepth);
    ...
    function get3DIteDepth(itemDepth, parentDepth) {
