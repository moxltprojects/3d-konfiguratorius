...
async function initConfigFunctions(
   ...
   function createFurnitureDimensionArrows(childMeshGroup, room3DGroup) {
    ...
    const positionMm = JSON.parse(childMeshGroup.userData.positionMm);
...
function makeTextSprite(message, params = {}) {
   ...
   // // Helper: simple text sprite
    ...
    // function createFurnitureItemControls(childMeshGroup, uiButtons, customId) {
    //     // 1. Remove old controls from this model
    //     if (uiButtons) {
    //         uiButtons.forEach(btn => childMeshGroup.remove(btn));
    //     }

    //     // 2. Get model size (for button placement)
    //     const bbox = new THREE.Box3().setFromObject(childMeshGroup);
    ...
    //         img.onload = () => {
    //             // Draw icon centered, 70% of canvas
    ...
    //         // const btn = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.5), material);
    //         const btn = new THREE.Mesh(new THREE.PlaneGeometry(buttonSize, buttonSize), material);
    //         btn.position.set(offsetX, size.y + topOffset, 0);
    //         btn.userData.isUIButton = true;
    //         btn.userData.actionType = actionType;
    //         btn.userData.customId = customId;

    //         btn.lookAt(threeJSCamera.position)

    //         return btn;
    //     }

    //     // 3. Create buttons
    //     const duplicateBtn = makeButton("duplicate.svg", -buttonSize, "duplicate");
    //     const deleteBtn   = makeButton("delete.svg", buttonSize, "delete");

    //     // 4. Attach to model
    //     childMeshGroup.add(duplicateBtn);
    //     childMeshGroup.add(deleteBtn);

    //     // Store for later cleanup
    //     childMeshGroup.userData.uiButtons = [duplicateBtn, deleteBtn];
    // }
    // function createFurnitureItemControls(childMeshGroup, uiButtons, customId) {
    //     // Remove old buttons
    //     allUIButtons = [];
    ...
    //     if (uiButtons) {
    //         childMeshGroup.userData.uiButtons?.forEach(btn => childMeshGroup.remove(btn));
    //     }

    //     const worldPos = new THREE.Vector3();
    //     childMeshGroup.getWorldPosition(worldPos);

    //     const size = childMeshGroup.userData.scaledSize.clone(); // model size

    //     const camDistance = worldPos.distanceTo(threeJSCamera.position);
    //     // const buttonSize = camDistance * 0.4; // tweak for visibility
    //     const referenceDistance = 5; // meters at which size = 1
    //     const baseButtonSize = 4; // meters
    //     const buttonSize = (camDistance / referenceDistance) * baseButtonSize;

    //     const buttonGap = camDistance * 1;
    //     const topOffset = size.y + buttonSize * 0.5; // always just above model top

    //     // Buttons X offset relative to center
    //     const offsetsX = [-buttonGap / 2, buttonGap / 2];

    //     function makeSpriteWithBackground(iconUrl, offsetX, actionType) {
    //         const canvasSize = 128;
    //         const canvas = document.createElement('canvas');
    //         canvas.width = canvasSize;
    //         canvas.height = canvasSize;
    ...
    //         const sprite = new THREE.Sprite(material);
    //         sprite.scale.set(buttonSize, buttonSize, 1);

    //         // Set position: x relative to model center, y just above top, z same as model
    //         sprite.position.set(worldPos.x + offsetX, worldPos.y + topOffset, worldPos.z);
    //         sprite.renderOrder = 999;
    //         sprite.layers.set(1); 
    //         threeJSCamera.layers.enable(1);
    ...
    //     const duplicateBtn = makeSpriteWithBackground("duplicate.svg", offsetsX[0], "duplicate");
    //     const deleteBtn = makeSpriteWithBackground("delete.svg", offsetsX[1], "delete");

    //     childMeshGroup.add(duplicateBtn);
    //     childMeshGroup.add(deleteBtn);
    //     childMeshGroup.userData.uiButtons = [duplicateBtn, deleteBtn];

    //     allUIButtons = [duplicateBtn, deleteBtn];
    // }
    ...
    //     // const size = childMeshGroup.userData.scaledSize.clone();
    //     // const worldPos = new THREE.Vector3();
    //     // childMeshGroup.getWorldPosition(worldPos);
    //     // const topY = worldPos.y + size.y; // top of the model

    //     // // Create a button sprite
    //     // function makeButton(iconUrl) {
    //     //     const canvasSize = 128;
    //     //     const canvas = document.createElement('canvas');
    //     //     canvas.width = canvasSize;
    //     //     canvas.height = canvasSize;
    ...
    //     //     ctx.arc(canvasSize/2, canvasSize/2, r, 0, Math.PI*2);
    //     //     ctx.fill();
    ...
    //     //         map: texture,
    //     //         transparent: true,
    //     //         depthTest: false  // ensures it always renders on top
    //     //     });
    ...
    //     //     // Initial position (will be updated in the render loop)
    //     //     sprite.position.set(worldPos.x, topY, worldPos.z);

    //     //     return sprite;
    //     // }

    //     // const btn1 = makeButton("duplicate.svg");
    //     // const btn2 = makeButton("delete.svg");

    //     // childMeshGroup.add(btn1);
    //     // childMeshGroup.add(btn2);
    //     // childMeshGroup.userData.uiButtons = [btn1, btn2];

    //     // allUIButtons.push(btn1, btn2); // Add to global array for update loop
...
function createFurnitureItemControlsOld(childMeshGroup, customId) {
    // Remove old buttons
    if (childMeshGroup.userData.uiButtons) {
        childMeshGroup.userData.uiButtons.forEach(btn => modelScene.remove(btn));
    }
    ...
    const buttonOffsetX = 0.05; // small horizontal offset (meters), adjust as needed

    function makeButtonMesh(iconUrl, offsetX) {
        const canvasSize = 128;
        const canvas = document.createElement('canvas');
        canvas.width = canvasSize;
        canvas.height = canvasSize;
        ...
        // Plane geometry for button
        const buttonSize = Math.max(size.x, size.y, size.z) * 0.25; // relative to model size
        ...
        mesh.userData.customId = customId;
        mesh.userData.actionType = iconUrl.includes('duplicate') ? 'duplicate' : 'delete';

        // Position: centered on X, top of model, small horizontal offset
        mesh.position.set(worldPos.x + offsetX, topY + buttonSize / 2, worldPos.z);

        // Make always face camera
        mesh.lookAt(threeJSCamera.position);
    ...
    const duplicateBtn = makeButtonMesh('duplicate.svg', -buttonOffsetX);
    const deleteBtn = makeButtonMesh('delete.svg', buttonOffsetX);
    ...
    //     const pixelGap = 15; // screen space gap in pixels
    //     const baseSize = Math.max(size.x, size.y, size.z) * 0.2; // base button size

    //     // Helper to create a button sprite
    //     function makeButton(iconUrl, offsetX) {
    //         const canvasSize = 128;
    //         const canvas = document.createElement('canvas');
    //         canvas.width = canvasSize;
    //         canvas.height = canvasSize;
    ...
    //         // Initial position; X offset is temporary
    //         sprite.position.set(worldPos.x + offsetX, topY, worldPos.z);

    //         // Add to scene (not as child of model)
    //         modelScene.add(sprite);

    //         return sprite;
    //     }

    //     // Compute X offset in world units (will be refined in update loop)
    //     const offset = 0.1; // small initial offset
    //     const btn1 = makeButton("duplicate.svg", -offset/2);
    //     const btn2 = makeButton("delete.svg", offset/2);
    ...
    //     function makeSpriteWithBackground(iconUrl, actionType) {
    //         const canvasSize = 128;
    //         const canvas = document.createElement('canvas');
    //         canvas.width = canvasSize;
    //         canvas.height = canvasSize;
    ...
    //     const duplicateBtn = makeSpriteWithBackground("duplicate.svg", "duplicate");
    //     const deleteBtn = makeSpriteWithBackground("delete.svg", "delete");

    //     childMeshGroup.add(duplicateBtn);
    //     childMeshGroup.add(deleteBtn);
    //     childMeshGroup.userData.uiButtons = [duplicateBtn, deleteBtn];

    //     allUIButtons.push(duplicateBtn, deleteBtn);

    //     // Position and size them immediately
    //     updateUIButtonPositions(childMeshGroup, childMeshGroup.userData.uiButtons, threeJSCamera, threeJSRendered);
    // }

    
    // function updateUIButtonPositions(childMeshGroup, buttons, camera) {
    //     const worldPos = new THREE.Vector3();
    //     childMeshGroup.getWorldPosition(worldPos);

    //     const size = childMeshGroup.userData.scaledSize.clone();
    //     const camDistance = worldPos.distanceTo(camera.position);

    //     const referenceDistance = 5;
    //     const baseButtonSize = Math.max(size.x, size.y, size.z) * 0.3;
    //     const minSize = baseButtonSize * 1;
    //     const maxSize = baseButtonSize * 5;
    //     const buttonSize = THREE.MathUtils.clamp(baseButtonSize * (camDistance / referenceDistance), minSize, maxSize);

    //     const modelTopY = worldPos.y + size.y;
    //     const spriteY = modelTopY - buttonSize / 2;

    //      // Horizontal right vector in XZ plane
    //     const cameraDir = new THREE.Vector3();
    //     camera.getWorldDirection(cameraDir);
    //     cameraDir.y = 0; // flatten to XZ plane
    //     cameraDir.normalize();

    //     // Vector pointing to the right of the camera (camera's local X)
    //     const cameraRight = new THREE.Vector3();
    //     camera.getWorldDirection(cameraRight);
    //     cameraRight.crossVectors(new THREE.Vector3(0,1,0), cameraDir).normalize();  // perpendicular to forward and up

    //     const pixelGap = 15; // pixels
    //     const fov = camera.fov * (Math.PI / 180);
    //     const height = 2 * Math.tan(fov / 2) * camDistance; // visible height at distance
    //     const width = height * camera.aspect;
    //     const worldGap = (pixelGap / threeJSRendered.domElement.clientWidth) * width; // convert px to world units

    //     buttons[0].position.copy(worldPos).addScaledVector(cameraRight, -worldGap / 2);
    //     buttons[1].position.copy(worldPos).addScaledVector(cameraRight, worldGap / 2);

    //     buttons.forEach(btn => btn.position.y = spriteY);
    //     buttons.forEach(btn => btn.scale.set(buttonSize, buttonSize, 1));
    // }


    function updateUIButtonSizes(buttons) {
        buttons.forEach(btn => {
            const childMeshGroup = btn.parent;
            // updateUIButtonPositions(childMeshGroup, buttons, threeJSCamera);
            // const parentSize = btn.parent.userData.scaledSize || new THREE.Vector3(1,1,1);
            // const worldPos = new THREE.Vector3();
            // btn.parent.getWorldPosition(worldPos);

            // const distance = worldPos.distanceTo(threeJSCamera.position);

            // // Base size relative to model dimensions (max dimension)
            // const modelMaxSize = Math.max(parentSize.x, parentSize.y, parentSize.z);
            // const baseSize = modelMaxSize * 0.2; // tweak scaling factor

            // // Scale with camera distance
            // const referenceDistance = 5; // distance at which baseSize is correct
            // const size = baseSize * (distance / referenceDistance);

            // btn.scale.set(size, size, 1);
        });
    }
    ...
    // function updateUIButtonPositions(buttons, camera, renderer) {
    //     const pixelGap = 15; // fixed gap in screen pixels
    ...
    //         const worldPos = new THREE.Vector3();
    //         btn.parent.getWorldPosition(worldPos);
    //         const size = btn.parent.userData.scaledSize.clone();
    //         const topY = worldPos.y + size.y;

    //         // Camera distance
    //         const distance = worldPos.distanceTo(camera.position);

    //         // Base size relative to model
    //         const modelMaxSize = Math.max(size.x, size.y, size.z);
    //         const baseSize = modelMaxSize * 2.5; // tweak: fraction of model size

    //         // Scale factor: 1 at referenceDistance, larger if camera farther
    //         const referenceDistance = 25; // meters
    //         let scaleFactor = distance / referenceDistance;

    //         // Clamp scale to avoid insane sizes
    //         scaleFactor = THREE.MathUtils.clamp(scaleFactor, 0.5, 1.5);

    //         const buttonSize = baseSize * scaleFactor;
    //         btn.scale.set(buttonSize, buttonSize, 1);

    //         // Convert pixel gap to world units
    //         const fov = camera.fov * (Math.PI/180);
    //         const height = 2 * Math.tan(fov/2) * distance;
    //         const width = height * camera.aspect;
    //         const worldGap = (pixelGap / renderer.domElement.clientWidth) * width;
    ...
    //         if (btn === buttons[0]) btn.position.addScaledVector(cameraRight, -worldGap/2);
    //         else btn.position.addScaledVector(cameraRight, worldGap/2);

    //         // Align top of button to top of model
    //         btn.position.y = topY + buttonSize/2;
    //     });
    // }
    ...
    //         // Use world X-axis to separate buttons
    //         const horizontalOffset = (i === 0 ? -1 : 1) * worldGap / 2;

    //         btn.position.set(
    //             worldPos.x + horizontalOffset,
    //             topY + buttonSize / 2, // align top of button to model top
    //             worldPos.z
    //         );
    //     });
    // }
    ...
    function updateUIButtonPositions(buttons, camera, renderer) {
        ...
        buttons.forEach((btn, i) => {
            ...
            btn.parent.getWorldPosition(worldPos);
            const size = btn.parent.userData.scaledSize.clone();
            const topY = worldPos.y + size.y; // top of model

            // --- Keep your size logic as-is ---
            const distance = worldPos.distanceTo(camera.position);
            const referenceDistance = 5;
            const baseSize = Math.max(size.x, size.y, size.z) * 1; // YOUR original
            const minScale = 1;
            const maxScale = 3.0;
            const scaleFactor = THREE.MathUtils.clamp(distance / referenceDistance, minScale, maxScale);
            const buttonSize = baseSize * scaleFactor;
            btn.scale.set(buttonSize, buttonSize, 1);

            // --- Positioning with fixed screen gap ---
            // Project top of model to screen
            const topVector = new THREE.Vector3(worldPos.x, topY, worldPos.z).project(camera);
            const widthHalf = renderer.domElement.clientWidth / 2;
            const heightHalf = renderer.domElement.clientHeight / 2;
            const screenX = topVector.x * widthHalf + widthHalf;
            const screenY = -topVector.y * heightHalf + heightHalf;

            const targetScreenX = i === 0 ? screenX - pixelGap/2 : screenX + pixelGap/2 + buttonSize;

            // Convert back to world
            const vectorNDC = new THREE.Vector3(
                (targetScreenX / renderer.domElement.clientWidth) * 2 - 1,
                - (screenY / renderer.domElement.clientHeight) * 2 + 1,
                topVector.z
            );
            vectorNDC.unproject(camera);

            btn.position.set(vectorNDC.x, vectorNDC.y, vectorNDC.z);
        });
    }
    ...
    function removeGLBModel(childObj) {
        ...
        // if (childObj.userData.uiButtons) {
        //      child.userData.uiButtons.forEach(btn => {
        //         child.remove(btn);
        //     });
        //     child.userData.uiButtons = null; 
        // }
