...
function initRoomConfigFunctions(
    ...
    async function initFurnitureRotation(roomType, currentCustomId) {
        ...
        const newRotation = baseRotation + Math.PI / 4;
    ...
    //             object = furnitureDimensions[DIMENSION_TYPE_BOTTOM];
    ...
    //             object = furnitureDimensions[DIMENSION_TYPE_TOP];
    ...
    //             object = furnitureDimensions[DIMENSION_TYPE_FULL];
    ...
    //     const itemHeight = object.height;
    //     const itemMinHeight = object.min_height;
    //     const itemDepth = object.depth;
    //     const itemMinDepth = object.min_depth;
    ...
    async function changeTotals() {
        total = changeProductsPrices(container.querySelectorAll('.my-cabinets-list .my-cabinet-item'), state.defaultTextures, state.defaultComponents);
    ...
    function changeModelChildrenDimensions(modelRoomType, furnitureType, newDimensions) {
        ...
        for (const childObj of children) {
            ...
            const { scaledChildSize } =
                placeFurnitureInRoom(modelRoomType, childObj, childFurnitureType, widthPx, heightPx, depthPx, userData.positionMm);
    ...
    function setCornerChildenDragging(modelRoomType, obj, furnitureType) {
        ...
        //     if(obj.userData.furnitureType === DIMENSION_TYPE_BOTTOM) {
        //         pos.z = Math.max(
        //             roomBox.min.z + (objWidth) + WALL_THICKNESS,
        //             Math.min(roomBox.max.z - objWidth / 2, pos.z)
        //         );
        //     } else {
        //         pos.z = clamp(
        //             pos.z,
        //             roomBox.min.z + objWidth / 2 + WALL_THICKNESS + cornerDepth,
        //             roomBox.max.z - objWidth / 2
        //         )
        //     }
        // } else {
        //     pos.x = clamp(
        ...
        // }
         ...
         if (getIsRotatedItem(yRotation)) {
            ...
            pos.x = Math.max(
              ...
              console.log("-----****-------")
                    console.log(obj.userData.customId)
                    console.log(obj.userData.productId)
        console.log(pos)
    ...
    function addExistingGLBModel(
        ...
        loader.load(
            ...
            function (childGltf) {
                ...
                console.log("**-*-*-*-*-")
                const { scaledChildSize } = placeFurnitureInRoom(modelRoomType, wrapper, furnitureType, widthPx, heightPx, depthPx, spaceBottomPx, childPositionMm, null, childRotation);
        ...
        return new Promise((resolve, reject) => {
            ...
            loader.load(
                ...
                function (childGltf) {
                    ...
                    wrapper.userData.depthMm = spaceBottomMm;
                    ...
                    console.log(customId)
                    console.log(productId)
                    const { scaledChildSize } = placeFurnitureInRoom(modelRoomType, wrapper, furnitureType, itemModelWidth, itemModelHeight, itemModelDepth, itemModelSpaceBottom);
                ...
                //    addFrontTopShade(wrapper, scaledChildSize);
                    ...
                    const itemPositionMm = getItemPosition(modelRoomType, furnitureType, placedWorldPos.clone(), scaledChildSize, wrapper.userData.savedPosition.y);
    ...
    function placeFurnitureInRoom(
        ...
        if (mmPosition) {
            ...
            const pos = typeof mmPosition === "string"
                ...
                : mmPosition;
            console.log(getIsRotatedItem(rotation))
             console.log(initRotation)
            if(initRotation && getIsRotatedItem(rotation)) { 
                console.log("YES");
                ...
                x = -roomHalfWidth + pos.left * modelRoomScale.x - min.x;
                z = -roomHalfDepth + pos.back * modelRoomScale.z - min.z;
            } else {
                const footprintSize = getFootprintSize(scaledSize, rotation);
                x = -roomHalfWidth
...
console.log(finalWorldPos)
    ...
    function getItemPosition(roomType, furnitureType, currentPosition, currentSize, spaceBottomPx, id = null) {
        ...
        addBtn.addEventListener('click', async function() {
            ...
            const itemDepth = item.getAttribute('data-item_depth');
             const itemSpaceBottom = item.getAttribute('data-item_space_bottom');
       		const hasBrandTexture = item.getAttribute('data-has_brand_texture');
    ...
    // function editGLBModelWidth(modelRoomType, customId, itemWidth, itemHeight, itemDepth) {
...
// //         const bottomPercent = (furnitureDimensions[DIMENSION_TYPE_BOTTOM].height + furnitureDimensions.space_bottom.current) / (roomHeightCm * 10);
