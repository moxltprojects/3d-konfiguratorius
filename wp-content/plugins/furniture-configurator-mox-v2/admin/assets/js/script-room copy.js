...
let 
    ...
    currentRoomModel = ROOM_TYPE_SINGLE_WALL,
    roomModels = {
        [ROOM_TYPE_SINGLE_WALL]: MODEL_OBJ_STRUCTURE,
        [ROOM_TYPE_WITH_CORNER]: MODEL_OBJ_STRUCTURE,
    };
...
function initRoomConfigFunctions(
    ...
    function changeRoomDimensionsValue() {
        ...
        dimensionsItemContainers.forEach(dimensionsContainer => {
            const type = dimensionsContainer.getAttribute('data-type');
    ...
    function changeFurnitureDimensionsValue() {
        ...
        dimensionsItemContainers.forEach(dimensionsContainer => {
            ...
            rangeInput.addEventListener('change', function(e) {
                ...
                changeModelChildrenDimensions(roomModels[ROOM_TYPE_SINGLE_WALL]);
                changeModelChildrenDimensions(roomModels[ROOM_TYPE_WITH_CORNER]);
    ...
    function changeRoomTypeTab(tab, content, modelContainer) {
        if(tab.classList.contains('active')) return;

        const currentTab = container.querySelector('.cabinet-settings .main-settings-sidebar .cabinets .room-type-tabs button.active');
        const currentContent = container.querySelector('.cabinet-settings .main-settings-sidebar .cabinets .tab-item.current');
        const currentModelContainer = container.querySelector( `.model-display-container .room-model-container-inner .canvas-parent.current`);
    ...
    function changeCabinetsTab(tab, contents) {
        ...
        const currentContents = container.querySelectorAll('.cabinet-settings .main-settings-sidebar .cabinets .content .content-inner > .tab-content.current');
    ...
    async function initFurnitureDuplicateData(modelObj, currentCustomId) {
        const currentObj = modelObj.modelChildren.find(child => child.userData.customId == currentCustomId);
    ...
    function initFurnitureRemoveData(modelObj, customId) {
        ...
        removeItemButtonTriggerAction(modelObj, furnitureItem, summaryItem, furnitureType, customId);
    }
    ...
    function removeItemButtonTriggerAction(modelObj, furnitureItem, summaryItem, furnitureType, customId) {
        const children = modelObj.modelChildren;
        const childObj = children.find(child => child.userData.customId == customId);
        removeGLBModel(modelObj, childObj, modelObj.modelScene);
    ...
    function removeItemButtonTrigger(furnitureItem, customId, furnitureType) {
     ...
     function duplicateFurnitureTrigger(modelObj, parent, customId) {
        ...
        furnitureButton.addEventListener('click', function() {
            duplicateFurniture(modelObj, customId);
        });
    }

    async function duplicateFurniture(modelObj, customId, currentCustomId, triggerDupItem = false) {
        if(stepsContainer) {
            stepsContainer.classList.add('loading');
        }
        ...
        const myItemsList = container.querySelector('.tab-item.current .my-cabinets-list .my-cabinets-list-inner');
        ...
        if(my_item_html) {
            ...
            if(furnitureItem) {
                editItemButtonTrigger(modelObj, furnitureItem); 
                removeItemButtonTrigger(modelObj, furnitureItem, customId, furnitureType);
                duplicateFurnitureTrigger(modelObj, furnitureItem, customId);
            }
        }
    ...
    function getItemDimensions(type) {
        ...
        switch(type) {
            case DIMENSION_TYPE_BOTTOM: {
                ...
                itemMinHeight = bottomHeightMin;
                itemMinDepth = bottomDepthMin;

                furnitureDimensions.bottom_depth = get3DItemHeight(itemDepth, modelRoomDepth);
                furnitureDimensions.bottom_height = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = furnitureDimensions.bottom_height;
                modelDepth = furnitureDimensions.bottom_depth;
            ...
            case DIMENSION_TYPE_TOP: {
                ...
                itemMinHeight = topHeightMin;
                itemMinDepth = topDepthMin;

                furnitureDimensions.top_depth = get3DItemHeight(itemDepth, modelRoomDepth);
                furnitureDimensions.top_height = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = furnitureDimensions.top_height;
                modelDepth = furnitureDimensions.top_depth;
                break;
            }
            case DIMENSION_TYPE_TOP_CORNER: {
                itemHeight = furnitureDimensions.top_height;
            ...
            case DIMENSION_TYPE_FULL: {
                ...
                itemMinHeight = fullHeightMin;
                itemMinDepth = fullDepthMin;

                furnitureDimensions.full_depth = get3DItemHeight(itemDepth, modelRoomDepth);
                furnitureDimensions.full_height = get3DItemHeight(itemHeight, modelRoomHeight);
                
                modelHeight = furnitureDimensions.full_height;
                modelDepth = furnitureDimensions.full_depth;
    ...
    function get3DItemHeight(itemHeight, parentHeight) {
        const heightWorld = (itemHeight / (roomHeight * 10)) * parentHeight;
        return heightWorld;
    }

    function get3DItemDepth(itemDepth, parentDepth) {
    ...
    //     // const {new_regular_total, new_total_discount, new_display_price, products_with_prices} = await calculateAllTotals(
    //     //     totalContainer,
    ...
    //     //     const discountPrice = priceData['total_discount'];
    ...
    function changeModelChildrenDimensions(modelObj) {
        ...
        for(let i = 0; i < children.length; i++) {
            ...
            let { itemHeight, modelHeight, itemDepth, modelDepth } = getItemDimensions(childFurnitureType);

            let widthPx = getItemWidthPx(itemWidth, modelRoomWidth)

            if(userData.pseudoType) {
                modelDepth = userData.depth_mm ? userData.depth_mm : modelDepth;
            } else {
                const scaleX = widthPx / originalSize.x;
                const scaleY = modelHeight / originalSize.y;
                const scaleZ = modelDepth / originalSize.z;
            ...
            if(itemIndex > -1) {
                ...
                const isFitting = checkIfAbleToDragChildToPosition(modelObj, childObj, scaledChildBox);
    ...
    function getCornerDimensions() {
        ...
        const bottomSpaceWorld = (space_bottom / (roomHeight * 10)) * modelRoomHeight;
    ...
    function init3dModel(model3dContainer, modelObj, containerWidth, containerHeight, roomType = ROOM_TYPE_SINGLE_WALL) {
        clearModelScene(model3dContainer, modelObj);
        const modelScene = new THREE.Scene();

        const textureLoader = new THREE.TextureLoader();
        const wallTexture = textureLoader.load(configDataRoom.generalAssetsUrl + '/images/brick-wall.jpg');
        const floorTexture = textureLoader.load(configDataRoom.generalAssetsUrl + '/images/floor-limestone.jpg');
       ...
       const threeJSRendered = new THREE.WebGLRenderer({ antialias: true, alpha: true, preserveDrawingBuffer: true });
        ...
        threeJSRendered.domElement.addEventListener('click', function(e) {
            onClickModel(e, newModelObj, modelScene, threeJSRendered, threeJSCamera);
        }, false);

        return newModelObj;
    }

    function removeAllFurnitureControls(modelScene) {
    ...
    function removeGLBModel(modelObj, childObj, modelScene) {
        if(!childObj) return;

        removeAllFurnitureButtons(modelScene);

        const childObjBox = new THREE.Box3().setFromObject(childObj);

        checkIfItemsAreFitting(modelObj, childObj, childObjBox);
        ...
        if(roomType === ROOM_TYPE_WITH_CORNER) {
            appendCornerPseudoModelObjects(room3DGroup);
        }
    }

    function setSingleWallChilderDragging(obj, modelObj) {
            const parentBoxWorld  = new THREE.Box3().setFromObject(modelObj.room3DGroup);
    ...
    function removeAllFurnitureButtons(modelScene) {
        const toRemove = [];
    ...
    function clearModelScene(model3dContainer, modelObj) {
        ...
        for (let i = children.length - 1; i >= 0; i--) {
            removeGLBModel(modelObj, children[i], modelScene, modelObj.room3DGroup);
        }
    ...
    function addExistingGLBModel(modelObj, id, urlSrc, furnitureType, productId, customId, itemWidth, originalSize, scaledSize, savedPosition, childPositionMm, isFitting) {
        const loader = new GLTFLoader();
        ...
        loader.load(
            ...
            function (childGltf) {
                ...
                initRoomDragging(modelObj, furnitureType, roomType, wrapper);
            },
            (xhr) => {
    ...
    function addGLBModel(
        ...
        return new Promise((resolve, reject) => {
            ...
            const itemModelWidth = getItemWidthPx(itemWidth, modelRoomWidth);
            ...
            loader.load(
                ...
                function (childGltf) {
                    ...
                    const { scaledChildBox, scaledChildSize } = normalizeAndPlaceMeshGroup(modelObj, wrapper, childBox, furnitureType, itemModelWidth, itemModelHeight, itemModelDepth);
                ...
                //    addFrontTopShade(wrapper, scaledChildSize);
                    ...
                    const isFittingItem = initModelDragging(modelObj, wrapper, childBox, scaledChildBox, scaledChildSize, customId, furnitureType);
                    ...
                    const tempModelObj = {...modelObj};
                    tempModelObj.dbChildren.push(furnitureListItem);
                    tempModelObj.modelChildren.push(wrapper);
                    roomModels = {...roomModels, [currentRoomModel]: {...tempModelObj}};
                    // roomState.dbChildren.push(furnitureListItem);
                    // roomState.modelChildren.push(wrapper);
    ...
    function normalizeAndPlaceMeshGroup(modelObj, childMeshGroup, targetBox, furnitureType, widthPx, heightPx, depthPx){
            ...
            const backZ = finalParentBox.min.z + (scaledChildSize.z / 2) + WALL_THICKNESS;  // offset from parent's center

            childMeshGroup.position.add(new THREE.Vector3(
                leftX - scaledChildBox.getCenter(new THREE.Vector3()).x,
                yPosition - scaledChildBox.getCenter(new THREE.Vector3()).y,
                backZ - scaledChildBox.getCenter(new THREE.Vector3()).z
            ));
    ...
    function initModelDragging(modelObj, childMeshGroup, childBox, scaledChildBox, scaledChildSize, customId, furnitureType) {
        ...
        if(!furnitureType.includes('corner')) {
            ...
            dragControls.addEventListener('drag', event => {
                ...
                let isFitting = true;
                if(roomType == ROOM_TYPE_WITH_CORNER) {
                    isFitting = setCornerChildenDragging(obj, modelObj);
                } else {
                    isFitting = setSingleWallChilderDragging(obj, modelObj);
                }
            ...
            checkIfItemsAreFittingForNewCorner(modelObj, childMeshGroup, childBox);
            removeCornerPseudoModelObjects(modelObj, furnitureType);
        }

        return isFittingItem;
    }

    function initRoomDragging(modelObj, furnitureType, roomType, wrapper) {
        /******* enable dragging *********/
    ...
    function removeCornerPseudoModelObjects(modelObj, furnitureType) {
        ...
        for(let i = 0; i < children.length; i++) {
            ...
            if(furnitureType === DIMENSION_TYPE_FULL) {
                tempI++;
                removeGLBModel(modelObj, childObj);
                continue;
    ...
    function getItemPosition(currentPosition, currentSize, id = null) {
        ...
        if(id) {
            const currentIndex = roomState.dbChildren.findIndex(item => item.db_data?.custom_id == id);
            if(currentIndex > -1) {
                roomState.dbChildren[currentIndex].db_data.furniture_position_mm = newPosition;
            }
        }
    ...
    function checkIfAbleToDragChildToPosition(modelObj, currentObj, currentBox, exludeObjId = null) {
    ...
    function checkIfItemsAreFitting(modelObj, currentObj, currentBox) {
        const currentId = currentObj.id;
        const currentObjType = currentObj.userData.furnitureType;
        ...
        if(roomType === ROOM_TYPE_SINGLE_WALL) {
            while(i < childrenCount) {
                ...
                if(
                    ...
                    const isFitting = checkIfAbleToDragChildToPosition(modelObj, childObj, childBox, currentId);
                    if(isFitting) {
    ...
    function onClickModel(e, modelObj, modelScene, threeJSRendered, threeJSCamera) {
            ...
            if (buttonHit) {
                ...
                if (action === "delete") {
                    initFurnitureRemoveData(modelObj, customId);
                } else if (action === "duplicate") {
                    initFurnitureDuplicateData(modelObj, customId);
                }
                return; 
            }
    
            removeAllFurnitureControls(modelScene);
            removeAllFurnitureButtons(modelScene);
    ...
    function initRoomCabinetTypeTabs() {
        ...
        tabs.forEach(tab => {
            ...
            const content = container.querySelector(`.cabinet-settings .main-settings-sidebar .tab-item[data-room_type="${type}"]`);
            const modelContainer = container.querySelector(
                `.model-display-container .room-model-container-inner .canvas-parent[data-type="${type}"]`
            );
            tab.addEventListener('click', function(e) {
                e.preventDefault(); 
                currentRoomModel = type;   
                changeRoomTypeTab(tab, content, modelContainer);
                editContainerRemove();
            });
        });
    }
    ...
    function initCabinetTabs() {
        ...
        tabs.forEach(tab => {
            const type = tab.getAttribute('data-type');
            const contents = container.querySelectorAll(`.cabinet-settings .main-settings-sidebar .cabinets .content .tab-content[data-type="${type}"]`);
    ...
    function furnitureTypeAddInit(
        ...
        addBtn.addEventListener('click', async function() {
            ...
            const { itemHeight, modelHeight, itemMinHeight, itemDepth, modelDepth, itemMinDepth } = getItemDimensions(furnitureType);
            ...
            const modelObj = roomModels[currentRoomModel];
            const {itemPositionMm} = await addGLBModel(
                modelObj,
                modelFileSrc, 
                furnitureType, 
    ...
    function checkIfItemsAreFittingForNewCorner(modelObj, currentObj, currentBox) {
        const currentId = currentObj.id;
        const currentObjType = currentObj.userData.furnitureType;
    ...
    async function initSettingsSave() {
        ...
        const createNewBtn = container.querySelector('.top-bar button[data-action_type="create-settings"]');
        ...
        function initButton(button, save = false) {
            button.addEventListener('click', async function(e) {
                ...
                // if(dataResponse) {
                //     if(!configId) currentConfigId = dataResponse.config_id;
                //     roomState.dbChildren = dataResponse.products_list;
                // }
...
async function getRoomAdminConfigComponentShortcodeContent() {
    try {
        console.log("TEST");
...
async function saveUserSettingsAjax(
    ...
    roomDimensions,
    furnitureDimensions,
    singleWallProductsList,
    doubleWallProductsList,
    // productsList
) {
    ...
    try {
        ...
        formData.append("products_list", JSON.stringify({
            ROOM_TYPE_SINGLE_WALL: singleWallProductsList,
            ROOM_TYPE_WITH_CORNER: doubleWallProductsList,
        }));
        ...
        function modChildrenList(products, type) {
            for(let i = 0; i < products.length; i++) {
                const listItem = {...products[i]};
                listItem.old_item = true;
                products[i] = {...listItem};
            }
            roomModels[type].dbChildren = products;
        }
